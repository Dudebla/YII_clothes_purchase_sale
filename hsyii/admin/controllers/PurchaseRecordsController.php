...
class PurchaseRecordsController extends BaseController {
    ...
    public function actionDelete($id) {
       ...
       //$id = $request->post('purchase_id'); 
        ...
        //找出account表中的账单id
        $modelAccount = AccountInformation::model()->find('purchase_id='.$id);      
        $accountId = $modelAccount->id;

        // Yii::app()->runController('AccountInformation/Delete','id'.$accountId);
        // parent::_clear($id);
        //删除account的记录
        $modelAccountName = AccountInformation::model()->model();
        $modelAccounts = $modelAccountName::model();
        $countAccount = $modelAccounts->deleteAll('id'.' in (' . $accountId . ')');


        //删除purchase的记录
        $modelPurchaseName = $this->model;
        $modelPurchase = $modelPurchaseName::model();
        $countPurchase = $modelPurchase->deleteAll('id'.' in (' . $id . ')');
        if ($countPurchase > 0 && $countAccount > 0 ) {
            ajax_status(1, '删除成功', '/PurchaseRecords/index');
        } else {
            ajax_status(0, '删除失败');
        }
    }
